  ...
  <main id="main" class="l-main--one-column">
    ...
    <section class="p-card-list__menu">
      <ul class="p-card-list__menu-list">
        <li class="p-card-list__menu-item active"><a href=""><i class="fas fa-envelope-open-text"></i>届いたカード</a></li>
    ...
    <section class="p-card-list__contents">
      ...
      <div class="c-sort-menu">
        <?php if (!$is_asc) { ?>
          <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam() . '&asc=1' : '?asc=1'; ?>"><i class="fas fa-sort-numeric-down c-sort-menu__icon"></i>日付の古い順に並べ替え</a>
        <?php } else { ?>
          <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam(array('asc')) : '?asc=0'; ?>"><i class="fas fa-sort-numeric-down-alt c-sort-menu__icon"></i>日付の新しい順に並べ替え</a>
        <?php } ?>
      </div>
      <div class="c-sort-menu">
        <?php if (!$is_fav) { ?>
          <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam() . '&fav=1' : '?fav=1'; ?>"><i class="fab fa-gratipay c-sort-menu__icon"></i>お気に入りカードのみ表示</a>
        <?php } else { ?>
          <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam(array('fav')) : '?fav=0'; ?>"><i class="fas fa-inbox c-sort-menu__icon"></i>すべてのカードを表示</a>
        <?php } ?>
      </div>
