...
<body>
  ...
  <div id="contents" class="site-width">
    ...
    <div class="sub-menu">
      <ul class="menu-list">
        <li class="menu-item active"><a href=""><i class="fas fa-envelope-open-text"></i>届いたカード</a></li>
    ...
    <section id="main" class="main page-1column">
        ...
        <div class="sort-icon">
          <?php if(!$is_asc){ ?>
            <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam().'&asc=1' : '?asc=1'; ?>"><i class="fas fa-sort-numeric-down icon icon-sort"></i>日付の古い順に並べ替え</a> 
          <?php }else{ ?>
            <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam(array('asc')) : '?asc=0'; ?>"><i class="fas fa-sort-numeric-down-alt icon icon-sort"></i>日付の新しい順に並べ替え</a> 
          <?php } ?>

          <?php if(!$is_fav){ ?>
            <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam().'&fav=1' : '?fav=1'; ?>"><i class="fab fa-gratipay icon icon-fav"></i>お気に入りカードのみ表示</a> 
          <?php }else{ ?>
            <a href="cardList.php<?php echo (!empty(appendGetParam())) ? appendGetParam(array('fav')) : '?fav=0'; ?>"><i class="fab fa-gratipay icon icon-fav"></i>すべてのカードを表示</a> 
          <?php } ?>
        </div>
