  ...
  <main id="main" class="l-main--one-column">
    ...
    <section class="p-card-list__menu">
      <ul class="p-card-list__menu-list">
        ...
        <li class="p-card-list__menu-item"><a href="cardList_sent.php"><i class="fas fa-envelope"></i>贈ったカード</a></li>
        <li class="p-card-list__menu-item active"><a href="cardList_all.php"><i class="fas fa-share-alt-square"></i>みんなのカード</a></li>
      </ul>
    </section>

    <section class="p-card-list__contents">
      <h2 class="c-title__sub"><i class="fas fa-share-alt-square"></i>みんなのカード</h2>
      <div class="c-sort-menu">
        <?php if (!$is_asc) { ?>
          <a href="cardList_all.php<?php echo (!empty(appendGetParam())) ? appendGetParam() . '&asc=1' : '?asc=1'; ?>"><i class="fas fa-sort-numeric-down c-sort-menu__icon"></i>日付の古い順に並べ替え</a>
        <?php } else { ?>
          <a href="cardList_all.php<?php echo (!empty(appendGetParam())) ? appendGetParam(array('asc')) : '?asc=0'; ?>"><i class="fas fa-sort-numeric-down-alt c-sort-menu__icon"></i>日付の新しい順に並べ替え</a>
        <?php } ?>
      </div>
      <div class="c-sort-menu">
        <?php if (!$is_fav) { ?>
          <a href="cardList_all.php<?php echo (!empty(appendGetParam())) ? appendGetParam() . '&fav=1' : '?fav=1'; ?>"><i class="fab fa-gratipay c-sort-menu__icon"></i>お気に入りカードのみ表示</a>
        <?php } else { ?>
          <a href="cardList_all.php<?php echo (!empty(appendGetParam())) ? appendGetParam(array('fav')) : '?fav=0'; ?>"><i class="fas fa-inbox c-sort-menu__icon"></i>すべてのカードを表示</a>
        <?php } ?>
      </div>
          ...
          <div class="p-card-panel__card-item">
            ...
            <div class="p-card-panel__card-info">
              <p class="p-card-panel__name">
                ...
                <?php endif; ?>
                <span class="u-font-size--s">さん &gt;&gt;</span>
