...
define('MSG16', 'いずれかを選択してください');
define('MSG17', 'ゲストユーザーのため、この操作はできません');
// 成功メッセージ
define('SUC01', 'パスワードを変更しました');
define('SUC02', 'プロフィールを変更しました');
define('SUC03', 'メールを送信しました');
define('SUC04', 'カードを贈りました！');
define('SUC05', 'カードを削除しました');
define('SUC06', 'ユーザー登録が完了しました！ようこそ！');
define('SUC07', '退会処理が完了しました。ご利用ありがとうございました');
define('SUC08', 'こんにちは！ゲストユーザーとしてログインしました');
define('SUC09', 'カードテンプレートを登録しました');
...
function getUsersPass($u_id)
{
  ...
  try {
    ...
    $dbh = dbConnect();
    // SQL文
    $sql = 'SELECT id, username, email, pass FROM users WHERE id = :u_id AND is_deleted = 0';
    $data = array(':u_id' => $u_id);
    // クエリ実行
    $stmt = queryPost($dbh, $sql, $data);
...
function getTemplate($c_id = '')
{
  ...
  try {
    ...
    $dbh = dbConnect();
    // SQL文
    $sql = 'SELECT id, card_pic, card_name, user_id FROM cards WHERE is_deleted = 0';
    $data = array();
...
//-------------------------------------------------
//相手とのやりとりを取得
function getConversations($u_id, $pn_id, $span = 20, $currentMinNum = 0, $is_asc = '', $is_fav = '')
{
  ...
  try {
    ...
    $sql .= ' WHERE to_user = :u_id AND from_user = :pn_id AND m.is_deleted = 0 OR to_user = :pn_id AND from_user = :u_id AND m.is_deleted = 0';
    $data = array(':u_id' => $u_id, ':pn_id' => $pn_id);
    // クエリ実行
    $stmt = queryPost($dbh, $sql, $data);
    ...
    $data = array(':u_id' => $u_id, ':pn_id' => $pn_id, ':span' => $span, ':currentMinNum' => $currentMinNum);
    // クエリ実行
...
function pagination($currentPageNum, $totalPageNum, $pageColNum = 5)
{
  ...
  // 1ページ目以外は先頭へ戻るボタンを表示
  if ($currentPageNum != 1) {
    echo '<li class="c-pagination__list-item"><a href="?p=1' . $link . '">&lt;</a></li>';
  }
  // リンクを生成（現在のページ番号にはactiveクラスを付与）
  for ($i = $minPageNum; $i <= $maxPageNum; $i++) {
    echo '<li class="c-pagination__list-item ';
    if ($currentPageNum == $i) {
      echo 'active';
    }
  ...
  if ($currentPageNum != $totalPageNum && $totalPageNum > 1) {
    echo '<li class="c-pagination__list-item"><a href="?p=' . $totalPageNum . $link . '">&gt;</a></li>';
  }
  echo '</ul>';
  echo '</div>';
}
